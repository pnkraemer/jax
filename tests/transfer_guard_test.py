...
if jax._src.lib.xla_extension_version >= 57:
...
if jax._src.lib.xla_extension_version < 57:
  ...
  class TransferGuardTest(jtu.JaxTestCase):
    ...
    def assertLogs(self, func_name):
      ...
      try:
        with transfer_guard_lib._TestingScopedLogSink() as log_sink:
          yield
        for line in log_sink.logs():
          # Simply search for a pattern that describes a transfer.
          # This requires all backends to include such a pattern, and may have
          # false negatives due to other log messages containing the pattern.
          if line.find(" transfer: ") != -1:
            break
        else:
          raise RuntimeError("Expected a transfer log message")
      except Exception as e:  # pylint: disable=broad-except
        raise RuntimeError(
            f"Expected a transfer to be logged while running: {func_name}"
        ) from e
