...
if lib.xla_extension_version >= 57:
...
if lib.xla_extension_version < 57:
  ...
  _ALL_TRANSFER_GUARD_NAMES = (
    # (name, transfer_guard_lib state key)
    ('jax_transfer_guard_host_to_device', 'host_to_device'),
    ('jax_transfer_guard_device_to_device', 'device_to_device'),
    ('jax_transfer_guard_device_to_host', 'device_to_host'),
  )

  def _update_all_transfer_guard_global(val):
    for name, _ in _ALL_TRANSFER_GUARD_NAMES:
  ...
  def transfer_guard(new_val: str) -> Iterator[None]:
    """Set up thread-local state and return a contextmanager for managing it."""
    with _transfer_guard(new_val):
      state = transfer_guard_lib.thread_local_state()
      prev_vals = []
      for name, key in _ALL_TRANSFER_GUARD_NAMES:
        prev_vals.append(getattr(_thread_local_state, name, unset))
        setattr(_thread_local_state, name, new_val)
        _update_transfer_guard(state, key, new_val)
      try:
        yield
      finally:
        for (name, key), prev_val in zip(_ALL_TRANSFER_GUARD_NAMES, prev_vals):
          if prev_val is unset:
            delattr(_thread_local_state, name)
            _update_transfer_guard(state, key, None)
          else:
            setattr(_thread_local_state, name, prev_val)
            _update_transfer_guard(state, key, prev_val)
